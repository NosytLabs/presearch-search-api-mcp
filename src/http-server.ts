...
export class PresearchHttpServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.use((err: Error, _req: express.Request, res: express.Response) => {
      logger.error('An unexpected error occurred', { error: err.message, stack: err.stack });
      res.status(500).json({
        jsonrpc: '2.0',
        id: null,
        error: {
   ...
   */
  private createConfigFromRequest(req: express.Request): Record<string, unknown> {
    ...
    if (queryConfig.requestTimeout) config.REQUEST_TIMEOUT = queryConfig.requestTimeout.toString();
    if (queryConfig.maxRetries) config.MAX_RETRIES = queryConfig.maxRetries.toString();
    if (queryConfig.rateLimit) config.RATE_LIMIT = queryConfig.rateLimit.toString();
    if (queryConfig.cacheTtl) config.CACHE_TTL = (Number(queryConfig.cacheTtl) * 1000).toString();
    if (queryConfig.debug) config.DEBUG = queryConfig.debug === 'true' ? 'presearch-mcp' : '';
    return config;
  }
   ...
   */
  private async handleMcpRequest(req: express.Request, res: express.Response, next: express.NextFunction): Promise<void> {
    try {
   ...
   */
  private async processMcpRequest(request: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const routeHandlers: Record<string, () => Promise<Record<string, unknown>>> = {
      'tools/list': async () => {
      ...
      'tools/call': async () => {
        ...
        const { name, arguments: args } = toolParams as { name: string; arguments: Record<string, unknown> };
    ...
    try {
      ...
      return {
        jsonrpc: '2.0',
        id,
        error: { code: -32602, message: 'Request processing error', data: (error as Error).message },
      };
    }
  }
  ...
  try {
    ...
    logger.info('Starting Presearch HTTP Server for Smithery deployment');
      ...
      logger.error('Failed to start HTTP server', { error: (error as Error).message });
      throw error;
    }
  }
