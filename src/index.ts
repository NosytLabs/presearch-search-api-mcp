...
async function main(): Promise<void> {
  try {
    ...
    process.on('uncaughtException', error => {
      const handledError = errorHandler.handleError(error, { operation: 'uncaught-exception' });
      logger.error('Uncaught exception', { error: handledError.message });
      process.exit(1);
    });

    process.on('unhandledRejection', reason => {
      const handledError = errorHandler.handleError(reason, { operation: 'unhandled-rejection' });
      logger.error('Unhandled rejection', { error: handledError.message });
      process.exit(1);
    });
  } catch (error) {
    const handledError = errorHandler.handleError(error, { operation: 'main-startup' });
...
if (
  ...
  main().catch(error => {
    const handledError = errorHandler.handleError(error, { operation: 'main-catch' });
