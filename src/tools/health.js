  ...
  execute: withErrorHandling(
    ...
    async (args, context) => {
      ...
      const apiKeyValidation = {
        ...
        guidance: config.apiKey ? getApiKeyGuidance(config.apiKey) : getApiKeyGuidance(""),
      };
      ...
      if (!apiKeyValidation.configured) {
        enhancedStatus.recommendations.push("Configure PRESEARCH_API_KEY environment variable to enable search functionality");
      ...
      if (!status.authenticated && apiKeyValidation.configured) {
        enhancedStatus.recommendations.push("Verify your API key at https://presearch.com/");
      }

      if (status.latencyMs > 5000) {
        enhancedStatus.recommendations.push("High latency detected - consider using a closer Presearch node");
      }
      ...
      return { 
        ...
        summary: {
          ...
          message: status.reachable && status.authenticated 
