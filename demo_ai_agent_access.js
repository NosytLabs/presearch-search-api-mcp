...
async function demonstrateAiAgentAccess() {
  ...
  try {
    ...
    if (researchTool && config.apiKey) {
      ...
      console.log(`   - Research summary: "${researchResults.research_summary?.substring(0, 150)}..."`);
      console.log(`   - Key findings: ${researchResults.key_findings?.length || 0} items`);
    }
    ...
    if (scrapeTool) {
      ...
      console.log('   📤 Output received:');
      console.log(`   - Scraped URLs: ${scrapeResults.length}`);
      if (scrapeResults[0]) {
        console.log(`   - URL: ${scrapeResults[0].url}`);
        console.log(`   - Title: ${scrapeResults[0].title}`);
        console.log(`   - Content length: ${scrapeResults[0].text?.length || 0} characters`);
      }
    }
    ...
    console.log('   • Files are saved locally when export tools specify filenames');
    console.log('   • AI can chain multiple tools for complex workflows');
